...
To fit the model to simulated data, we follow the same process as in the "Simple Example" vignette. For that reason, we do not show the data generation steps here.
...
We now fit a BCF model to the simulated data. This is the same model as we fit in the "Simple Example" vignette. 
...
We now use a separate set of test values for prediction. First, we generate a testing dataset.
...
```{r}
pred_out = bcf2::predict_bcf(bcf_out=bcf_out,
...
```{r}
# Computing the average X3 values of Q1 and Q4 for prediction
q1 <- x[,3][x[,3] < quantile(x[,3], 0.25)]
q4 <- x[,3][x[,3] > quantile(x[,3], 0.75)]
...
pred_low = bcf2::predict_bcf(bcf_out=bcf_out,
...
pred_high = bcf2::predict_bcf(bcf_out=bcf_out,
...
distDF <- data.frame(taus  = c(lowDist, highDist),
                     Group = factor(c(rep(paste0("X3 = ", round(mean(q1), 1)), length(lowDist)), rep(paste0("X3 = ", round(mean(q4), 1)), length(highDist)))))
