...
We now use a separate set of testing Xs for prediction. First, we generate a testing dataset.
We purposefully make sure that we have some values outside of the range of our training dataset to allow us to 
gauge out-of-sample error.
...
We found that there is a 100% probability that the mean marginal treatment effect for top-quartile units is greater than that for bottom-quartile units.
