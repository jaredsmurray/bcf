...
In this vignette, we show our we can use the BCF package to fit a model and use the fitted object to predict estimates for new data.
...
We will use the same data as our training set for prediction. This will allow us to assess whether BCF predicts accurately. [MMF: Hm... I hate to throw a wrench in the stew at this late stage, but I  vote that we omit this check. What do you think? When I've read other package vignettes, they haven't typically consisted of checks that the package is working as advertised. Instead, I've usually seen worked examples of typical use cases. With that in mind, what would think of including instead an example where we predict for new Xs? I'm imagining a scatterplot of tau vs. X3, with posterior means and 95% CIs for tau shown as points and lines respectively. Datapoints included in the original fit in one color, new datapoints we're predicting for in anther color. How does that sound to you?]
...
upper <- c(apply(bcf_out$tau, 2, quantile, 0.975), apply(pred_out$tau, 2, quantile, 0.025))
...
In this plot, we notice that the credible intervals for the testing group are narrower than those for the training group, and that the predictions follow closely the results from our testing data.