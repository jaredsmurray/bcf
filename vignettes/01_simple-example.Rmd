...
# create targeted selection, whereby a unit's likelihood of joining the intervention (pi) 
# is related to its expected outcome (mu)
...
We use the `summarise_bcf` function to obtain posterior summary statistics and MCMC diagnostics. We use those diagnostics to assess convergence of our run.

```{r diagnostics}
bcf2::summarise_bcf(bcf_out)
```

Since our "$\hat{R}$" (Gelman and Rubin's convergence factor or Potential Scale Reduction Factor) are between 0.9 and 1.1, we are confident that our run successfully achieved convergence. 
...
First, we look at how well our estimates of $\tau$ recover the "truth."
...
Here we show a comparison of the average treatment effect for units in the top quartile of $X_3$ vs. the average treatment effect for units in the bottom quartile of $X_3$.
...
groupTaus <- data.frame(taus     = c(wq1Taus, wq4Taus),
                        subgroup = c(rep("q1", 4*n_sim), rep("q4", 4*n_sim)))
...
We found that there is a 100% probability that the average treatment effect for top-quartile units (q4) is greater than that for bottom-quartile units (q1).
